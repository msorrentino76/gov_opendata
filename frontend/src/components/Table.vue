...
<template>
    ...
    <el-table
        ...
        style="width: 100%"
        :height="props.heightTab ? props.heightTab : 480"
        ...
        <!-- ATTACHMENTS -->
        <el-table-column v-if="attachments && attachments.field && !attachments.hideOnColumn"
        ...
        <!-- ACTION -->    
        <el-table-column align="right" v-if="actions && (actions.customA || actions.customB || actions.read || actions.update || actions.delete)">
            ...
            <template #default="scope">
                ...
                <!-- CUSTOM -->
                <el-tooltip v-if="actions && actions.customA" class="box-item" effect="dark" :content="actions.customA ? actions.customA.content : ''" placement="top-start">
                    <el-button
                        :type="actions.customA.type"
                        :icon="actions.customA.icon"
                        circle 
                        @click="actions.customA.callback(scope.row.id)"
                        :disabled="actions.customA.disabled ? actions.customA.disabled(scope.$index, scope.row) : false"
                        />
                </el-tooltip>

                <el-tooltip v-if="actions && actions.customB" class="box-item" effect="dark" :content="actions.customB ? actions.customB.content : ''" placement="top-start">
                    <el-button
                        :type="actions.customB.type"
                        :icon="actions.customB.icon"
                        circle 
                        @click="actions.customB.callback(scope.row.id)"
                        :disabled="actions.customB.disabled ? actions.customB.disabled(scope.$index, scope.row) : false"
                        />
                </el-tooltip>

                <el-tooltip v-if="actions && actions.read" class="box-item" effect="dark" content="Dettagli" placement="top-start">
                    <el-button
                        type="primary"
                        :icon="Search"
                        circle 
                ...
                <el-tooltip v-if="actions && actions.update" class="box-item" effect="dark" content="Modifica" placement="top-start">
                    <el-button
                    type="warning"
                    :icon="Edit"
                    circle
                ...
                <el-popconfirm v-if="actions && actions.delete" title="Procedere con la cancellazione?" @confirm="handleDelete(scope.$index, scope.row)" confirm-button-text="Si">
                    <template #reference>
                            <el-button
                                type="danger" 
                                :icon="Delete"
...
const formatter = (row, column) => {
    ...
    if(format == 'singleselect') {
        return header.source.filter(obj => obj.value === value).map(obj => obj.label)
    }

    if(format == 'multiselect') {
        return header.source.filter(obj => value.includes(obj.value) ).map(obj => obj.label).join('\n');
    }
